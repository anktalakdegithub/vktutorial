<div class="sa4d25">
    <div class="container">
        ...
        <div class="row">
         ...
         if(in_array("add", $course) || in_array("all", $course)){
           ?>
                   ...
                   <a href="<?=base_url();?>admin/course/add_questionwrite_submitted?qw_id=<?=$qw_id;?>&batch_id=<?=$batch_id;?>" class="btn btn-primary">
                    Add Submission
                </a>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <div class="ui search focus lbel25 d-flex">
                                                    ...
                                                    <div class="ui left icon">
                                                            <input type="checkbox"  id="qwabsent_<?=$stu['id']?>" class="is_absent mx-2" name="is_absent">
                                                    </div>
                                                </div>
                                                <div class="ui search focus lbel25 mt-30 d-flex">
                                                    <label>Is Submitted</label>
                                                    <div class="ui left icon">
                                                            <input type="checkbox" id="qwsubmitted_<?=$stu['id']?>" class="is_submitted mx-2"  name="is_submitted">
                                                    </div>
                                                </div>
                                                <div class="ui search focus lbel25 mt-30">
                                                    <label>Remark</label>
                                                    <div class="ui left icon input swdh19">
                                                        <input class="prompt srch_explore" type="text" id="qwremark_<?=$stu['id']?>"
                                                            value="<?=$stu['remark']?>">
                                                    </div>
                                                </div>
                                                <div id="qmsg"></div>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="button" class="btn btn-primary updatequestionw" value="<?=$stu['id']?>">Update</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary"
                                                    data-dismiss="modal">Close</button>
                                                <button type="button" class="btn btn-primary deleteqw" value="<?=$stu['id']?>">Yes</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        var qwabsent="";
        if( $('#qwabsent_'+aid).attr( 'type' ) === 'checkbox' ) {
            qwabsent += $('#qwabsent_'+aid).is( ':checked' ) ? 1: 0;
        }
        var qwsumitted="";
        if( $('#qwsubmitted_'+aid).attr( 'type' ) === 'checkbox' ) {
            qwsumitted += $('#qwsubmitted_'+aid).is( ':checked' ) ? 1: 0;
        }

        var qwremark=$('#qwremark_'+aid).val();
        formData.append('qwabsent', qwabsent);
        formData.append('qwsumitted', qwsumitted);
        formData.append('qwremark', qwremark);
        formData.append('aid', aid);
        $.ajax({
            url: "<?= base_url()?>admin/course/updatequestionw",
            data: formData,
            type: "post",
            headers: { 'IsAjax': 'true' },
            processData: false,
       ...
       // 
       $('body').on('click', '.deleteqw', function(){ 
        var aid=$(this).val();
        
        $.ajax({
            url: "<?= base_url()?>admin/course/deleteqwsingle",
            data: {aid:aid},
            type: "post",
            success: function(data){
                 swal("Question Writing deleted successfully!", "", "success");
                    setTimeout(function () {
                        swal.close();
                      location.reload();
                    }, 1000); 
