...
<div class="row">
   <div class="col-md-8 col-12">
      <h2 class="st_title"><i class="uil uil-analysis"></i> Assignment Submitted</h2>
      <br>
   </div>
...
<div class="row">
   <div class="col-md-12 order-12 order-md-1">
      <div class="row" id="load_data">
        <table class="table">
            ...
            <tbody id="students">
                ...
                foreach ($students as $stud) {
                  ?>
                  <tr class="students" data-id="<?=$stud->StudentId;?>">
                    <th scope="col"><?=$i;?></th>
        ...
        <button class="btn btn-default steps_btn" name="save" id="saveassignment" value="<?=$assignment_id;?>">Save</button>
      </div>
   </div>
</div>
</div>
<script type="text/javascript">
   $('#saveassignment').click(function(){
    // alert();
        ...
        $.ajax({
        url: "<?= base_url()?>admin/course/save_assignment_submitted",
        data: {'sids':sids,'is_late':is_late,'assignment_id':assignment_id,'is_submitted':is_submitted,'remark':remark},
        type: "post",
        success: function(data){
          // alert();
         location.href="<?=base_url();?>admin/course/assignment_submit?assignment_id=<?=$assignment_id;?>&batch_id=<?=$batch_id;?>";
        }
        });
   });
</script>